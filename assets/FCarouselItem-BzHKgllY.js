import{j as r}from"./jsx-runtime-Cf8x2fCZ.js";import{r as n}from"./index-DY-snNUy.js";const a={"f-carousel":"_f-carousel_d5w12_1","f-carousel__viewport":"_f-carousel__viewport_d5w12_8","f-carousel__track":"_f-carousel__track_d5w12_12","f-carousel__track--animating":"_f-carousel__track--animating_d5w12_17","f-carousel__item":"_f-carousel__item_d5w12_20","f-carousel__arrow":"_f-carousel__arrow_d5w12_26","f-carousel__arrow--left":"_f-carousel__arrow--left_d5w12_43","f-carousel__arrow--right":"_f-carousel__arrow--right_d5w12_46","f-carousel__indicators":"_f-carousel__indicators_d5w12_60","f-carousel__indicator":"_f-carousel__indicator_d5w12_60","f-carousel__indicator--active":"_f-carousel__indicator--active_d5w12_75","f-carousel-item":"_f-carousel-item_d5w12_79"},N="data:image/svg+xml;base64,",Z="data:image/svg+xml;base64,",z=({children:d,visibleCount:t=1,showArrows:m=!0,loop:o=!1,autoplay:p=!1,autoplayInterval:y=3e3,className:M})=>{const[f,i]=n.useState(0),[c,_]=n.useState(!1),s=d.length,b=n.useRef(null),I=n.useRef(null);n.useEffect(()=>{if(p)return I.current=setInterval(()=>{_(!0),i(e=>e>=s-t?o?0:e:e+1)},y),()=>{I.current&&clearInterval(I.current)}},[p,y,s,t]),n.useEffect(()=>{if(c){const e=setTimeout(()=>_(!1),500);return()=>clearTimeout(e)}},[c]),n.useEffect(()=>{i(0)},[s,t]);const g=()=>{c||(_(!0),i(e=>e===0?o?s-t:0:e-1))},h=()=>{c||(_(!0),i(e=>e>=s-t?o?0:e:e+1))},u=n.useRef(null),v=e=>{u.current=e.touches[0].clientX},w=e=>{if(u.current===null)return;const l=u.current-e.touches[0].clientX;Math.abs(l)>50&&(l>0?h():g(),u.current=null)},j=()=>{u.current=null},T=[a["f-carousel"],M].filter(Boolean).join(" "),x=[a["f-carousel__track"],c?a["f-carousel__track--animating"]:""].filter(Boolean).join(" "),S={transform:`translateX(-${100/s*f}%)`,transition:c?"transform 0.6s cubic-bezier(.4,0,.2,1)":"none",width:`${100*s/t}%`};return r.jsxs("div",{className:T,children:[m&&r.jsx("button",{className:`${a["f-carousel__arrow"]} ${a["f-carousel__arrow--left"]}`,onClick:g,disabled:!o&&f===0,"aria-label":"Назад",children:r.jsx("img",{src:N,alt:"Предыдущий слайд"})}),r.jsx("div",{className:a["f-carousel__viewport"],onTouchStart:v,onTouchMove:w,onTouchEnd:j,children:r.jsx("div",{ref:b,className:x,style:S,children:d.map((e,l)=>r.jsx("div",{className:a["f-carousel__item"],style:{width:`${100/s}%`},children:e},l))})}),m&&r.jsx("button",{className:`${a["f-carousel__arrow"]} ${a["f-carousel__arrow--right"]}`,onClick:h,disabled:!o&&f>=s-t,"aria-label":"Вперёд",children:r.jsx("img",{src:Z,alt:"Следующий слайд"})}),r.jsx("div",{className:a["f-carousel__indicators"],children:Array.from({length:s-t+1}).map((e,l)=>r.jsx("button",{className:[a["f-carousel__indicator"],l===f?a["f-carousel__indicator--active"]:""].join(" "),onClick:()=>i(l),"aria-label":`Перейти к слайду ${l+1}`},l))})]})};z.__docgenInfo={description:`Горизонтальный карусель с поддержкой:
- ручной и автоматической прокрутки
- свайпов на мобильных устройствах
- индикаторов текущего слайда
- циклической прокрутки

@component
@example
<FCarousel
  visibleCount={3}
  showArrows={true}
  loop={false}
  autoplay={true}
  autoplayInterval={4000}
>
  <div>Слайд 1</div>
  <div>Слайд 2</div>
  <div>Слайд 3</div>
</FCarousel>

@param {ReactNode[]} children - Список слайдов.
@param {number} [visibleCount=1] - Количество видимых слайдов одновременно.
@param {boolean} [showArrows=true] - Отображать ли стрелки навигации.
@param {boolean} [loop=false] - Включить зацикливание при достижении конца.
@param {boolean} [autoplay=false] - Автопрокрутка слайдов.
@param {number} [autoplayInterval=3000] - Интервал автопрокрутки в мс.
@param {string} [className] - Кастомный класс для внешнего контейнера.

@returns {JSX.Element} — Рендерит карусель с кнопками и точками-индикаторами.`,methods:[],displayName:"FCarousel",props:{children:{required:!0,tsType:{name:"Array",elements:[{name:"ReactNode"}],raw:"ReactNode[]"},description:"Массив дочерних элементов (обычно слайды в виде JSX)."},visibleCount:{required:!1,tsType:{name:"number"},description:`Количество одновременно отображаемых элементов.
@default 1`,defaultValue:{value:"1",computed:!1}},showArrows:{required:!1,tsType:{name:"boolean"},description:`Отображение стрелок навигации (вкл/выкл).
@default true`,defaultValue:{value:"true",computed:!1}},loop:{required:!1,tsType:{name:"boolean"},description:`Включает цикличную прокрутку (зацикливание).
@default false`,defaultValue:{value:"false",computed:!1}},autoplay:{required:!1,tsType:{name:"boolean"},description:`Автоматическое переключение слайдов.
@default false`,defaultValue:{value:"false",computed:!1}},autoplayInterval:{required:!1,tsType:{name:"number"},description:`Задержка автопрокрутки в миллисекундах.
@default 3000`,defaultValue:{value:"3000",computed:!1}},className:{required:!1,tsType:{name:"string"},description:"Пользовательский CSS класс для контейнера карусели."}}};const R=({children:d,className:t,id:m})=>r.jsx("div",{className:`${a["f-carousel-item"]} ${t||""}`,id:m,children:d});R.__docgenInfo={description:`Компонент \`FCarouselItem\` — отдельный элемент карусели.

Используется внутри \`FCarousel\` как обёртка для каждого слайда.

@component
@example
<FCarouselItem className="custom-slide">
  <img src="/slide1.jpg" alt="Слайд 1" />
</FCarouselItem>

@param {React.ReactNode} children - Контент слайда.
@param {string} [className] - Пользовательский класс.
@param {string} [id] - HTML ID.

@returns {JSX.Element} — Рендерит \`<div>\` со стилями карусели и кастомным классом.`,methods:[],displayName:"FCarouselItem",props:{children:{required:!0,tsType:{name:"ReactNode"},description:"Контент слайда (обычно изображение или JSX)."},className:{required:!1,tsType:{name:"string"},description:"Кастомный CSS класс для дополнительной стилизации."},id:{required:!1,tsType:{name:"string"},description:`Уникальный идентификатор элемента.
@type {string | undefined}`}}};export{R as F,z as a};
