...
`,e.jsx(d.pre,{children:e.jsx(d.code,{className:"language-typescript",children:`import { useState } from 'react';
import FSelectSearchDb from './FSelectSearchDb';
...
const MySearchComponent = () => {
  ...
  const fetchCities = async (text: string): Promise<City[]> => {
    // Имитация запроса к API
    const response = await fetch(\`/api/cities?search=\${text}\`);
    return response.json();
  };

  const handleSelectedElement = (item: City | null | undefined) => {
  ...
  return (
    <FSelectSearchDb<City>
      ...
      fetchingFunc={fetchCities}
      selectedElement={handleSelectedElement}
      selectItem={(item: City) => item.name}
      helpText="Введите минимум 3 символа для поиска"
      fullWidth
      type="search"
    />
  );
...
`]})]})}function f(r={}){const{wrapper:d}={...i(),...r.components};return d?e.jsx(d,{...r,children:e.jsx(l,{...r})}):l(r)}export{f as default};
